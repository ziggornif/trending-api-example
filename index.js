...
async function callGithubTrending(language, since = 'today') {
  try {
    ...
    for (var element of elements) {
      const project = await element.$eval('.h3', s => s.textContent.replace(/(\r\n|\n|\r|\s)/gm, "").trim());
...
app.get('/api/trending', async (req, res) => {
  res.json({
    languages: {
      values: [
        'javascript', 'python', 'css', 'typescript', 'go', '...'
      ],
      example: 'http://localhost:3000/api/trending/javascript',
    },
    since: {
      values: [
        'daily', 'weekly (default value)', 'monthly'
      ],
...
app.get('/api/trending/:language', async (req, res) => {
  const results = await callGithubTrending(req.params.language, req.query.since);
  res.json(results);
});
